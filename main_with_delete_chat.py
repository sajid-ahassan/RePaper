...
CHECKPOINT_DB_PATH = os.getenv(
    'CHECKPOINT_DB_PATH',
    r'C:\Users\sajid\OneDrive\Desktop\RePaper\src\DB\chatbot.db'
)
...
def get_qdrant_client():
    """Create a Qdrant client using the same env style as the rest of the app."""
    ...
    # Use this only if you run Qdrant locally. For Docker, set QDRANT_URL instead.
    return QdrantClient(path=os.getenv('QDRANT_LOCAL_PATH', './qdrant_storage'))


def delete_qdrant_session(thread_id):
    """Delete all Qdrant vectors that belong to one chat/session.

    The app stores documents by calling add_paper(session_id=active_thread, docs=docs).
    Most LangChain/Qdrant setups store that inside payload metadata.session_id.
    A second delete is attempted for plain session_id in case your vector_store.py stores it there.
    """
    client = get_qdrant_client()
    if client is None:
        return

    payload_keys = ['metadata.session_id', 'session_id']
...
def delete_langgraph_thread(thread_id):
    """Best-effort cleanup for LangGraph SQLite checkpoints/history.

    This removes rows from any table in the checkpoint DB that has a thread_id column.
    If your graph uses a different checkpointer, this function safely does nothing.
    """
    ...
    try:
        ...
        # Do not block JSON/Qdrant deletion only because checkpoint cleanup failed.
        pass


def delete_chat(thread_id):
    """Delete a chat from session.json, Qdrant, and local LangGraph history."""
    if thread_id not in st.session_state['session_meta']:
        return

    try:
        delete_qdrant_session(thread_id)
        delete_langgraph_thread(thread_id)
    except Exception as e:
        st.sidebar.error(f"Could not delete Qdrant data: {e}")
        return

    st.session_state['session_meta'].pop(thread_id, None)
    save_session(st.session_state['session_meta'])

    # If the deleted chat was open, move to another chat or create a fresh one.
